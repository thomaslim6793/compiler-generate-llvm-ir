...
and 'a expr =
  ...
  | ENil of 'a
  | EId of string * 'a
  (* We changed the semantics such that the first argument of a function call is the function closure. I.e.
     every function is now a lambda expression in a binding, and every lambda expression is called via its closure. 

     The function call for lambda expressions requires a closure containing bindings for its free vars within the lambda 
     expression which are bound to bindings that were defined before the lambda binding. I.e. these "free variables" are 
     free within the lambda, but they are bound in the greater expression which contains the binding of this lambda.
      
     E.g. 'let x = 1 in let f1 = lambda y : y + x in f1(2)', f1 is bound to a lambda containing a non-function parameter 'x' which
     is the name of the binding 'let x = 1' which is declared before 'let f1 = lambda ...'. 
     I.e. the closure for f1 is (arity: 1, code-pointer: RSP - some offset, num of non-function params: 1, free-var1: x).
     
     For regular function declarations, a closure isn't really needed, since it has no free variables in the function body,
     so we will give it a trivial closure - i.e. a closure with no free variables. 
     
     *Note* We won't be passing in an actual tuple AST object representing a function closure as the first argument of 
     our function call. I.e. the first argument of EApp is NOT an ETuple object representing closure.  
     Instead we will still be just passing in the bind name of the lambda expression as the first argument of a function call. 
     So then how does a function call access the closure of the function?
     Well, the function call simply calls the bind name of the lambda expression (since all functions are now lambdas), 
     and the assembly instructions of the lambda label in the assembly will contain the necessary instructions for 
     accessing the closure values. The actual closure of the lambda expression is constructed in the compilation of lambda expression.
     *)
  | EApp of 'a expr * 'a expr list * call_type * 'a 
  (* we changed the semantics of functions to be first-class, which basically means that a function evaluates to 
     some "object" where this object is something called 'closure'. I.e. a function is now considered an expression,
     just like how primitive values, operations, and function calls are expressions. 

     *Note* The idea of a closure is actually real simple: 
     CLOSURE OF LAMBDA EXPRESSION = LAMBDA EXPRESSION + ENVIRONMENT OF ALL NON-PARAMETER VARS OF LAMBDA EXPRESSION

     *Note* Now it makes sense to call a lambda an expression rather than a function, and this is why lambda is a variant
     of type expr as you can see it here.

     Every function is then uniformly desugared into a lambda binding, and we will compile lambda expression into its body
     and its closure. 
     
     *Note* When we call a lambda, we will not be calling its body because then the body alone has free variables. 
     Instead when we call a lambda, we will be calling its closure so that the body of the lambda expression has a proper 
     environment which contains all the bindings for its variables. 

     *Note* Because every lambda expression has a closure, when we compile a lambda expression, we will construct
     a label and a sequence of assembly instructions for constructing its closure.  
  *)
