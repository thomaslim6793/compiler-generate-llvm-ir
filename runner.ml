...
let run_asm (asm_string : string) (out : string) (runner : string -> string list -> string -> (string, string) result) args (std_input : string) =
  let outfile = open_out (out ^ ".ll") in
...
type compile_opts = { valgrind: bool; heap_size: int option }
...
let test_err ?args:(args=[]) ?std_input:(std_input="") program_str outfile errmsg ?vg:(vg=false) test_ctxt =
  let full_outfile = "output/" ^ outfile in
...
let test_does_err filename test_ctxt =
  ...
  let input = if Sys.file_exists infile then (string_of_file infile) else "" in
  test_err ~args:args ~std_input:input prog ("do_err/" ^ filename) err ~vg:opts.valgrind test_ctxt
...
let input_file_test_suite () =
  let safe_readdir dir ext =
    ...
    with _ -> [] in
  "input-file-suite">:::[
      "do_pass"  >:::(List.map (fun f -> f>::test_does_run f)   (safe_readdir "input/do_pass"   ".fdl"));
      "do_err"   >:::(List.map (fun f -> f>::test_does_err f)   (safe_readdir "input/do_err"    ".fdl"));
      "dont_pass">:::(List.map (fun f -> f>::test_doesnt_run f) (safe_readdir "input/dont_pass" ".fdl"));
      "dont_err" >:::(List.map (fun f -> f>::test_doesnt_err f) (safe_readdir "input/dont_err"  ".fdl"))
    ]
